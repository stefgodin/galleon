...
def draw_boat(game_state: gs.GameState, render_target: pygame.Surface, boat: en.Entity, layer: int):
    if layer == DRAW_LAYERS.ENTITY:
        ...
        pygame.draw.rect(surface= render_target, color= 'black', rect= [x - (health_bar_width/2) - 2, y - 12, health_bar_width + 4, 14])
        ...
        if game_state.show_boxes:
            ...
            for tile in neihbors:
                ...
                pygame.draw.lines(render_target, pygame.Color(255, 0, 0), True, [
                    (x - s/2, y - s/2),
                    (x + s/2, y - s/2),
                    (x + s/2, y + s/2),
                    (x - s/2, y + s/2),
